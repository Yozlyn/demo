...
const selectedRows = ref<MarketingActivity[]>([]);
...
const filteredTableData = computed(() => {
  return state.tableData.filter(item => {
    const searchMatch = item.activityName.includes(searchQuery.value) || 
                       item.activityId.includes(searchQuery.value);
...
// 表格选择和批量删除
const handleSelectionChange = (val: MarketingActivity[]) => { selectedRows.value = val; };
...
const handleBatchDelete = async () => {
  ...
  ElMessageBox.confirm(`确定要删除${selectedRows.value.length}个选中的活动吗？`, '警告', {
